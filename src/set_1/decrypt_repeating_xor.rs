...
/// Decrypts the given [`Base64`]-encoded ciphertext, which has been encrypted
/// with `repeating-key XOR`, returning a [`Metadata`] and plaintext pair.
///
/// Returns `None` if a key could not be found (e.g., empty input).
///
/// [`Base64`]: https://en.wikipedia.org/wiki/Base64
#[must_use]
pub fn decrypt_repeating_xor(b64: &[u8]) -> Option<(Metadata, Vec<u8>)> {
    if b64.is_empty() {
        return None;
    }

    let ciphertext = utils::b64_to_bytes(b64);
...
mod tests {
    ...
    fn test_decrypt_repeating_xor_file() {
        ...
        let (meta, plaintext) = decrypt_repeating_xor(&encrypted).unwrap();
