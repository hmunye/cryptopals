use super::{decrypt_xor_cipher::decrypt_xor_cipher, repeating_xor::repeating_xor, utils};
...
#[must_use]
pub fn decrypt_repeating_xor(input: &[u8]) -> (Option<Metadata>, String) {
    if input.is_empty() {
        return (None, String::new());
    }

    let bytes = utils::b64_to_bytes(input);
    let k = find_candidate_keysize(&bytes);

    let blocks = bytes.len() / k;
    ...
    let mut key = String::with_capacity(transposed.len());
    ...
    let plaintext = String::from_utf8(repeating_xor(&bytes, key.as_bytes()))
        .expect("Base64-encoded string should always be valid UTF-8");

    (Some(Metadata { key }), plaintext)
}

#[inline]
fn find_candidate_keysize(input: &[u8]) -> usize {
    let mut keysize = 0;
    ...
    for k in keyspace {
        let n = 10;
        let blocks: Vec<_> = input.chunks_exact(k).take(n).collect();
        ...
        // Average and normalize the distance for the given `k`, so smaller `k`s
        // are not biased.
...
mod tests {
    ...
    // Challenge 1-6
    #[test]
    fn test_decrypt_repeating_xor_file() {
        let encrypted = fs::read("encrypted_repeating_xor.txt").unwrap();

        let (meta, plaintext) = decrypt_repeating_xor(&encrypted);
        let meta = meta.unwrap();

        assert_eq!(meta.key, "Terminator X: Bring the noise");
        assert_eq!(
            plaintext,
